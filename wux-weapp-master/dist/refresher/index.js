...
baseComponent({
    ...
    computed: {
        classes: ['prefixCls, pullingText, pullingIcon, disablePullingRotation, refreshingText, refreshingIcon, visible, active, refreshing, tail, prefixLCls, loading, noData',
            function(prefixCls, pullingText, pullingIcon, disablePullingRotation, refreshingText, refreshingIcon, visible, active, refreshing, tail, prefixLCls, loading, noData) {
        ...
        onScroll(n) {
            ...
            query.select(`#${this.id}`).boundingClientRect((res) => {
                ...
                if (windowHeight && !this.isRefreshing()) {
                    // 到临界点时触发上拉加载
                    // 防止节点高度一致时引发重复加载
                    // console.log("当前滚动高度：",n, "newContentHeight:",newContentHeight,"oldContentHeight:",oldContentHeight, "windowHeight:",windowHeight,
                    //     "比较： ",n*1.2,"n>",newContentHeight - windowHeight- (distance*1.5), loading, "noData:",noData, )
                    if (
                        n*1.2 > newContentHeight - windowHeight - (distance*1.5) &&
                        loading === false &&
